                        ...
                        @if ($viewData['daKetThuc'])
                            @foreach ($viewData['ketQuaThi']->baiLam->chiTietBaiLams as $index => $ct)
                                <div class="review-question-card {{ $loop->first ? 'review-active-question' : '' }}"
                                    ...
                                    ">
                                            <input type="radio" id="q{{ $loop->iteration }}a"
                                                name="q{{ $loop->iteration }}" disabled
                                                {{ $ct->dapAnChon == $ct->hienThiA ? 'checked' : '' }}>
                                            <label for="q{{ $loop->iteration }}a">A.
                                                {{ $ct->cauHoi->{'dapAn' . $ct->hienThiA} }}</label>
                                    ...
                                    ">
                                            <input type="radio" id="q{{ $loop->iteration }}b"
                                                name="q{{ $loop->iteration }}" disabled
                                                {{ $ct->dapAnChon == $ct->hienThiB ? 'checked' : '' }}>
                                            <label for="q{{ $loop->iteration }}b">B.
                                                {{ $ct->cauHoi->{'dapAn' . $ct->hienThiB} }}</label>
                                    ...
                                    ">
                                            <input type="radio" id="q{{ $loop->iteration }}c"
                                                name="q{{ $loop->iteration }}" disabled
                                                {{ $ct->dapAnChon == $ct->hienThiC ? 'checked' : '' }}>
                                            <label for="q{{ $loop->iteration }}c">C.
                                                {{ $ct->cauHoi->{'dapAn' . $ct->hienThiC} }}</label>
                                    ...
                                    ">
                                            <input type="radio" id="q{{ $loop->iteration }}d"
                                                name="q{{ $loop->iteration }}" disabled
                                                {{ $ct->dapAnChon == $ct->hienThiD ? 'checked' : '' }}>
                                            <label for="q{{ $loop->iteration }}d">D.
                                                {{ $ct->cauHoi->{'dapAn' . $ct->hienThiD} }}</label>
                        ...
                        @else
                            @if (!empty($viewData['ketQuaThi']) && !empty($viewData['ketQuaThi']->baiLam->chiTietBaiLams))
                                @foreach ($viewData['ketQuaThi']->baiLam->chiTietBaiLams as $index => $ct)
                                    <div class="review-question-card {{ $loop->first ? 'review-active-question' : '' }}"
                                        data-question-index="{{ $index }}">
                                        <p class="review-question-text">Câu {{ $loop->iteration }}:
                                            {{ $ct->cauHoi->noiDung }}
                                        </p>
                            ...
                            @if ($viewData['daKetThuc'])
                                @foreach ($viewData['ketQuaThi']->baiLam->chiTietBaiLams as $index => $ct)
                                    @php
                                        ...
                                        } elseif (!empty($ct->dapAnChon) && $ct->dapAnChon != $ct->cauHoi->dapAnDung) {
