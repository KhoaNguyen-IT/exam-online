        ...
        @if ($viewData['deThi'] && $viewData['baiLam'] && $viewData['chiTietBaiLams'])
            <div class="exam-sidebar-left">
                <div class="exam-info-section">
                    @if ($viewData['deThi']->kyThi)
                        ...
                        <span style="font-size: 0.9em;">Bắt đầu lúc:
                            {{ \Carbon\Carbon::parse($viewData['deThi']->kyThi->ngayThi)->format('d/m/Y - H \g\i\ờ i \p\h\ú\t') }}</span>
                        <p class="exam-instruction-text mt-1">Thời gian làm bài: {{ $viewData['deThi']->thoiLuongPhut }} phút
                        </p>
                    @endif
            ...
            <div class="exam-question-main-area">
                <form id="exam-quiz-form" method="POST"
                    ...
                    @csrf
                    <input type="hidden" name="ngayThiKetQuaThi" value="{{ $viewData['deThi']->kyThi->ngayThi }}">

                    <input type="hidden" name="maBaiLam" id="maBaiLamBackup" value="{{ $viewData['baiLam']->maBL }}">
                                    ...
                                    @if (!empty($ct->cauHoi->hinhAnh))
                                        <div class="exam-question-image-container">
                                            <img src="{{ asset('storage/' . $ct->cauHoi->hinhAnh) }}"
                                                alt="Hình ảnh câu hỏi" class="exam-question-image">
                                        </div>
                                    @endif

                                    <div class="exam-options-group">
                                        <label class="exam-option-item">
                                            <input type="radio" name="question[{{ $ct->maCH }}]"
                                                value="{{ $ct->hienThiA }}" class="exam-option-radio"
                                                onchange="luuTamBaiLam({{ $ct->maCH }}, '{{ $ct->hienThiA }}')"> A.
                                            {{ $ct->cauHoi->{'dapAn' . $ct->hienThiA} }}
                                        </label>
                                        <label class="exam-option-item">
                                            <input type="radio" name="question[{{ $ct->maCH }}]"
                                                value="{{ $ct->hienThiB }}" class="exam-option-radio"
                                                onchange="luuTamBaiLam({{ $ct->maCH }}, '{{ $ct->hienThiB }}')"> B.
                                            {{ $ct->cauHoi->{'dapAn' . $ct->hienThiB} }}
                                        </label>
                                        <label class="exam-option-item">
                                            <input type="radio" name="question[{{ $ct->maCH }}]"
                                                value="{{ $ct->hienThiC }}" class="exam-option-radio"
                                                onchange="luuTamBaiLam({{ $ct->maCH }}, '{{ $ct->hienThiC }}')"> C.
                                            {{ $ct->cauHoi->{'dapAn' . $ct->hienThiC} }}
                                        </label>
                                        <label class="exam-option-item">
                                            <input type="radio" name="question[{{ $ct->maCH }}]"
                                                value="{{ $ct->hienThiD }}" class="exam-option-radio"
                                                onchange="luuTamBaiLam({{ $ct->maCH }}, '{{ $ct->hienThiD }}')"> D.
